...
int rand() {
    for (int i = 1; i < 3; i++) now = (now * aa + bb) % MOD;
    return now;
}
...
struct cmp {
    ...
    bool operator()(const T4& a, const T4& b) const {
        return *(a.data) < *(b.data);
    }
...
void copy_test() {
    sjtu::priority_queue<T, cmp> q;
    int num = 5000;
    for (int i = 1; i <= num; i++) q.push(T(rand()));
    for (int j = 1; j <= 5; j++) {
        sjtu::priority_queue<T, cmp> t(q);
        for (int i = 1; i <= 100; i++) t.push(T(rand()));
        for (int k = 1; k <= 10; k++) {
            std::cout << t.top() << ' ' << t.size() << ' ' << t.empty()
                      << std::endl;
            t.pop();
        }
        ...
        p = t;
        p = p = p = p;
        for (int i = 1; i <= 100; i++) p.push(T(rand()));
        for (int k = 1; k <= 10; k++) {
            std::cout << p.top() << ' ' << p.size() << ' ' << p.empty()
                      << std::endl;
            p.pop();
        }
    }
